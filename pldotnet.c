...
Datum pldotnet_inline_handler(PG_FUNCTION_ARGS)
{
	...
	char*  block_neo3 = CODEBLOCK;

	char* source_code = (char*) malloc(strlen(block_neo1) + strlen(block_neo2) + strlen(block_neo3) \
	    + strlen(block_neo4) + 1);
	sprintf(source_code, "%s%s%s%s", block_neo1, block_neo2, block_neo3, block_neo4);
