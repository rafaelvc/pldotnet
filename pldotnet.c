...
static int get_size_nullable_header(int argNm_size, Oid arg_type);
static void build_nullable_header(char* src,Datum* argname, Oid* argtype, int nargs);
...
const char resu_nullable_value[] = "libArgs.resu.value = resu_nullable.GetValueOrDefault();";
const char resu_nullable_flag[] = "libArgs.resu.is_null = !resu_nullable.HasValue;";
const char null_prop_suffix[] = ".is_null";
const char value_prop_suffix[] = ".value";
...
pldotnet_build_block4(Form_pg_proc procst)
{
    ...
    if (nargs == 0)
    {
         ...
         sprintf(block2str, "%s%s%s", resu_var, func, endFun);
         return block2str;
    }
 ...
 */
static int
get_size_nullable_header(int argNm_size, Oid arg_type)
{
    int total_size = 0;
    char question_mark = '?';
    char equal_char = '=';
    ...
    switch (arg_type){
        case BOOLOID:
            /* template: bool? <arg>=<arg>_nullable.is_null? (bool?)null : <arg>_nullable.value; */
            total_size = strlen(pldotnet_getNetNullableTypeName(arg_type)) + argNm_size
                + sizeof(equal_char) + argNm_size + strlen(nullable_suffix) + strlen(null_prop_suffix)
                + sizeof(question_mark) + sizeof(parenthesis_char) + strlen(pldotnet_getNetNullableTypeName(arg_type))
                + sizeof(parenthesis_char) + strlen(null_str) + sizeof(colon_char) + argNm_size + strlen(nullable_suffix)
                + strlen(value_prop_suffix) + sizeof(semicolon_char);
            break;
    }
 ...
 */
static void
build_nullable_header(char* src,Datum* argname, Oid* argtype, int nargs)
{
    /* template:
     *   bool? <argname>_nullable=<argname>.is_null? (bool?)null : <argname>.value;
     */
    char *h_ptr, *argNm;
    int h_size = 0, i;
    ...
    for (i = 0; i < nargs; i++)
    {
        ...
        switch (argtype[i]){
            case BOOLOID:
                sprintf(h_ptr,"%s%s=%s%s%s?(%s)null:%s%s%s;", pldotnet_getNetNullableTypeName(argtype[i])
                    , argNm, argNm, nullable_suffix, null_prop_suffix, pldotnet_getNetNullableTypeName(argtype[i])
                    , argNm, nullable_suffix, value_prop_suffix);
        }
...
pldotnet_build_block5(Form_pg_proc procst, HeapTuple proc)
{
    ...
    for (i = 0; i < nargs; i++) 
    {
        ...
        header_size += get_size_nullable_header(strlen(argNm),argtype[i]);
...
pldotnet_getNetTypeName(Oid id) 
{
    switch (id){
        case BOOLOID:
            return "Nullable_bool"; // System.Boolean
        case INT4OID:
...
pldotnet_CreateCStrucLibArgs(FunctionCallInfo fcinfo, Form_pg_proc procst)
{
    ...
    for (i = 0; i < fcinfo->nargs; i++)
    {
        ...
        {
            case BOOLOID:
                nullableP = (nullable_bool *) curArg;
                //TODO: Address arg conversion for different PGVERSION
                nullableP->value = fcinfo->argnull[i]? false : DatumGetBool(fcinfo->arg[i]);
                nullableP->is_null = fcinfo->argnull[i];
                break;
            case INT4OID:
...
pldotnet_getResultFromDotNet(char * libArgs, Oid rettype,FunctionCallInfo fcinfo)
{
    ...
    switch (rettype){
        case BOOLOID:
            nullableP = (nullable_bool *)(libArgs + dotnet_info.typeSizeOfParams);
            if(nullableP->is_null)
            {
                fcinfo->isnull=true;
                return (Datum) 0;
            } else {
                return  BoolGetDatum  (nullableP->value);
            }
        case INT4OID:
